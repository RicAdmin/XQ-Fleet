...
import { RentalAgreementDocument } from '#/lib/document-components'

export const Route = createFileRoute('/api/documents/agreement/$rentalId')({
  server: {
    handlers: {
      GET: async ({ request, params }) => {
        ...
        if (row.status !== 'active' && row.status !== 'closed') {
          return new Response('Agreement not available for this rental status', { status: 400 })
        }
        ...
        const element = createElement(RentalAgreementDocument, { data: docData }) as any
        const buffer = await renderToBuffer(element)

        const isDownload = new URL(request.url).searchParams.get('download') === '1'
        const filename = `agreement-${row.carPlateNumber ?? params.rentalId}.pdf`
